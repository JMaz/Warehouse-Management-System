...
public class DatabaseAPI {
	...
	//return 1 if location already occupied by item num
	//return 2 if location already occupied by other item num
	//return 0 if transaction successfull
	...
	//return 1 if location already occupied by item num
		...
		//return 0 if transaction successfull
		//return -1 if nothing happens
		public int assignLocationToItem(int i, String l,int q){
			int j= 0;
			try{
			ps = conn.prepareStatement("Select * FROM " + inventoryTable + " WHERE LocationNumber = ?");
			ps.setString(1,l);
			...
			if (j == i)
				...
				ps = conn.prepareStatement("INSERT INTO " + inventoryTable + " (ItemNumber,LocationNumber,qty) VALUES (?,?,?)");
				ps.setInt(1,i);
				ps.setString(2,l);
				ps.setInt(3,q);
				ps.executeUpdate();
	...
	public static void main(String[] args) throws SQLException {
		...
		int k = -2;
		k = test.assignLocationToItem(5555, "A203",222);
		
		if(k == 1)
			System.out.println("Already Exists");
		else if (k == 2)
			System.out.println("Location already Ocupided");
		else if (k == 0)
